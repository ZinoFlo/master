...
## 2026-06-17 - Mitigating Transitive Dependency Vulnerabilities via Overrides
**Vulnerability:** Moderate vulnerabilities in `follow-redirects` (GHSA-r4q5-vmmm-2653) and `qs` (GHSA-q8mj-m7cp-5q26) were identified. These are transitive dependencies of `http-server`.
**Learning:** Using the `overrides` field in `package.json` provides a declarative and persistent way to enforce safe versions of transitive dependencies, ensuring they don't regress during subsequent `npm install` cycles.
**Prevention:** Regularly run `npm audit` and use `overrides` in `package.json` to lock transitive dependencies to secure versions when they cannot be updated via the direct dependency.
