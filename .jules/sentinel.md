...
## 2026-07-12 - Transitive Dependency Vulnerabilities Mitigated via Overrides
**Vulnerability:** Moderate vulnerabilities in `follow-redirects` (GHSA-r4q5-vmmm-2653, auth header leakage) and `qs` (GHSA-q8mj-m7cp-5q26, DoS) introduced by `http-server`.
**Learning:** Using the `overrides` field in `package.json` is an effective way to pin safe versions of transitive dependencies without needing to wait for the parent package to update. It ensures a clean `npm audit` report and provides immediate protection.
**Prevention:** Regularly run `npm audit` and use `overrides` to address vulnerabilities in transitive dependencies that cannot be updated directly.
